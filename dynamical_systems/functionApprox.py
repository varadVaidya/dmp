...
class functionApprox():
    ...
    def create_Gaussian_basis(numBasis,firstorderSystem,ifPlot=True):
        ...
        center = np.exp(-firstorderSystem.alpha * des_activation)
...
if __name__ == '__main__':
    ...
    PSI,gaussian_values = functionApprox.create_Gaussian_basis(numBasis,phaseSystem,ifPlot = False)
    ...
    approximatedFunc = np.sum(weighted_evaluate,axis=1) * xvalues/np.sum(gaussian_values,axis=1)
    # print("approximated func",approximatedFunc)
    import matplotlib.pyplot as plt
